...
export default function Footer() {
  ...
  const brandTitle = footer?.brand_title || "DYNOTIX";
  const brandSubtitle = footer?.brand_subtitle || "EVENT PLATFORM";
  ...
  const contactTitle = footer?.contact_title || "Kontak";
  const contactAddress =
    footer?.contact_address ||
    "Jl. Raya Tajur, Kp. Buntar RT.02/RW.08, Kel. Muara sari, Kec. Bogor Selatan, Kota Bogor, Jawa Barat 16137";
  const contactEmail = footer?.contact_email || "dynotix@gmail.com";
  const contactPhone = footer?.contact_phone || "+62 831-6922-1045";

  const socialTitle = footer?.social_title || "Ikuti Kami";
  const socialDescription =
    footer?.social_description || "Tetap terhubung untuk update event terbaru";

  const facebookUrl = footer?.facebook_url || "#";
  const instagramUrl = footer?.instagram_url || "#";
  const twitterUrl = footer?.twitter_url || "#";
  const linkedinUrl = footer?.linkedin_url || "#";

  const copyrightText =
    footer?.copyright_text ||
    "© 2025 DYNOTIX Event Platform. All rights reserved.";
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div>
            ...
            <ul className="space-y-2">
              <li className="flex items-start gap-2 text-[#B3CFE5]/80 text-xs">
                <svg
                  className="w-4 h-4 text-[#4A7FA7] flex-shrink-0 mt-0.5"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          <div>
            ...
            <p className="text-[#B3CFE5]/80 text-xs mb-3">{socialDescription}</p>
            <div className="flex gap-2">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-8 h-8 bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg flex items-center justify-center text-white hover:bg-white/20 transition-all hover:scale-110 hover:shadow-lg"
              >
        ...
        <div className="border-t border-[#4A7FA7]/20 pt-4">
          <p className="text-[#B3CFE5]/70 text-xs text-center">{copyrightText}</p>
        </div>
      </div>
    </footer>
  );
